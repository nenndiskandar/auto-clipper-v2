...
class CaptionMakerSettingsPage(BaseProviderSettingsPage):
    ...
    def create_provider_content(self):
        ...
        # --- Faster-Whisper Local Model Settings ---
    ...
    def _create_faster_whisper_section(self):
        ...
        ctk.CTkLabel(sep, text="⚡ Local Faster-Whisper (Offline)", 
            font=ctk.CTkFont(size=11, weight="bold")).pack(anchor="w", padx=4, pady=(4, 4))
        
        # Transcription mode: API vs Local
        mode_frame = ctk.CTkFrame(sep, fg_color="transparent")
        mode_frame.pack(fill="x", padx=4, pady=(0, 4))
        
        ctk.CTkLabel(mode_frame, text="Transcription Mode:", font=ctk.CTkFont(size=11)).pack(anchor="w")
        
        self.transcription_mode_var = ctk.StringVar(value="api")
        mode_menu = ctk.CTkOptionMenu(mode_frame, 
            values=["api", "local"],
            variable=self.transcription_mode_var, height=26,
            command=self._on_transcription_mode_changed)
        mode_menu.pack(fill="x", pady=(4, 0))
    ...
    def _on_transcription_mode_changed(self, value):
        ...
        if mode == "local":
            ...
            if model_bin.exists() and config_json.exists():
                self.fw_status_label.configure(
                    ...
                    text_color="green")
                self.fw_download_btn.configure(state="normal")
            else:
                self.fw_status_label.configure(
            ...
            self.fw_status_label.configure(
                text="Using Whisper API (cloud-based)",
                text_color="gray")
            self.fw_download_btn.configure(state="disabled")
    ...
    def load_config(self):
        ...
        self.transcription_mode_var.set(fw_settings.get("mode", "api"))
