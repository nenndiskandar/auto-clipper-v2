...
class ReplizSettingsSubPage(BaseSettingsSubPage):
    ...
    def create_content(self):
        """Create page content"""
        # Why Repliz card
        why_card = ctk.CTkFrame(self.content, fg_color=("#e8f5e9", "#1b5e20"), corner_radius=5, border_width=1, border_color=("#2a2a30", "#2a2a30"))
...
Affordable: Only $1.74/month (29,000 IDR) for Premium plan"""
        ...
        ctk.CTkLabel(why_card, text=why_text, justify="left",
            font=ctk.CTkFont(size=11), wraplength=480).pack(anchor="w", padx=4, pady=(0, 8))
        
        # Sign up CTA
        signup_frame = ctk.CTkFrame(self.content, fg_color=("gray85", "gray20"), corner_radius=5, border_width=1, border_color=("#2a2a30", "#2a2a30"))
        signup_frame.pack(fill="x", pady=(0, 8))
        ...
        ctk.CTkButton(signup_frame, font=ctk.CTkFont(size=11), text="Sign Up for Repliz", height=22,
            fg_color=("#00A878", "#00A878"), hover_color=("#008F66", "#008F66"),
            command=lambda: webbrowser.open("https://s.id/ytrepliz"), text_color=("#0B0B0C", "#0B0B0C")).pack(fill="x", padx=4, pady=(0, 8))
        
        # API Configuration section
        config_section = ctk.CTkFrame(self.content, fg_color=("gray90", "gray17"), corner_radius=5, border_width=1, border_color=("#2a2a30", "#2a2a30"))
        config_section.pack(fill="x", pady=(0, 8))
        ...
        ctk.CTkLabel(access_frame, text="Your Repliz API Access Key", 
            font=ctk.CTkFont(size=11), text_color="gray").pack(anchor="w", pady=(4, 4))
        
        self.access_key_entry = ctk.CTkEntry(access_frame, height=24,
        ...
        self.secret_key_entry = ctk.CTkEntry(secret_frame, height=24, show="*",
        ...
        self.validate_btn = ctk.CTkButton(btn_frame, font=ctk.CTkFont(size=11), text="Validate Keys", height=22,
            fg_color=("#00A878", "#00A878"), command=self.validate_keys, text_color=("#0B0B0C", "#0B0B0C"))
        self.validate_btn.pack(side="left", fill="x", expand=True, padx=(0, 4))
        
        # Connected Accounts section (initially hidden)
        self.accounts_section = ctk.CTkFrame(self.content, fg_color=("gray90", "gray17"), corner_radius=5, border_width=1, border_color=("#2a2a30", "#2a2a30"))
        ...
        self.accounts_list.pack(fill="both", expand=True, padx=4, pady=(0, 8))
    ...
    def _display_accounts(self, data, show_popup=False):
        ...
        if total_accounts > 0:
            self.accounts_section.pack(fill="x", pady=(0, 8))
    ...
    def _create_account_card(self, account, index):
        ...
        if picture_url:
            ...
            ctk.CTkLabel(card, text=icon, font=ctk.CTkFont(size=22)).pack(pady=(6, 4))
        
        ctk.CTkLabel(card, text=platform_type.upper(), 
            font=ctk.CTkFont(size=11, weight="bold"),
            fg_color=("gray80", "gray30"), corner_radius=5,
            padx=4, pady=4).pack(pady=(4, 4))
        ...
        ctk.CTkLabel(card, text=status_text,
            ...
            corner_radius=5, padx=4, pady=4).pack(side="bottom", pady=(0, 6))
    ...
    def _load_profile_picture(self, parent, url, fallback_icon):
        ...
        ctk.CTkLabel(parent, text="...", font=ctk.CTkFont(size=15)).pack(pady=(6, 4))
        threading.Thread(target=do_load, daemon=True).start()
    ...
    def _display_profile_picture(self, parent, ctk_img):
        ...
        ctk.CTkLabel(parent, image=ctk_img, text="").pack(pady=(6, 4))
    ...
    def _display_fallback_icon(self, parent, icon):
        ...
        ctk.CTkLabel(parent, text=icon, font=ctk.CTkFont(size=22)).pack(pady=(6, 4))
